...
// 可选城市列表（出现过的城市，去重；默认含上海）
function citiesAvailable() {
  const db = getDb();
  const set = new Set(['上海', '杭州']); // 支持的城市基础集（即使暂无数据也可切换）
  for (const t of ['events', 'news', 'nav_links', 'communities', 'resources']) {
    try { for (const r of db.prepare(`SELECT DISTINCT city FROM ${t} WHERE city != ''`).all()) set.add(r.city); } catch {}
  }
  return [...set];
}

function newsList({ region = '', city = '', limit = 50 } = {}) {
  const db = getDb();
  const w = [], a = [];
  if (city) { w.push('city = ?'); a.push(city); }
  if (region) { w.push('region = ?'); a.push(region); }
  const where = w.length ? 'WHERE ' + w.join(' AND ') : '';
  a.push(limit);
  const rows = db.prepare(`SELECT * FROM news ${where} ORDER BY COALESCE(NULLIF(published_at,''), created_at) DESC, id DESC LIMIT ?`).all(...a);
  return rows.map((r) => { let tags = []; try { tags = JSON.parse(r.tags || '[]'); } catch {} return { ...r, tags }; });
}
function newsRegions(city = '') {
  const db = getDb();
  return db.prepare(`SELECT region, COUNT(*) n FROM news WHERE region != '' ${city ? 'AND city = ?' : ''} GROUP BY region ORDER BY n DESC`).all(...(city ? [city] : []));
}

// 活动：默认未来在前（未定日期排末尾）；past=true 取往期(倒序)；可按城市过滤。
function eventsList({ region = '', city = '', past = false, all = false, limit = 300 } = {}) {
  const db = getDb();
  const w = [], a = [];
  if (city) { w.push('city = ?'); a.push(city); }
  if (region) { w.push('region = ?'); a.push(region); }
  if (!all && past) w.push("start_at != '' AND start_at < date('now')");
  else if (!all) w.push("(start_at = '' OR start_at >= date('now'))");
  const where = w.length ? 'WHERE ' + w.join(' AND ') : '';
  a.push(limit);
  const order = past || all ? 'ORDER BY start_at DESC, id DESC' : "ORDER BY (start_at = '') ASC, start_at ASC, id ASC";
  const rows = db.prepare(`SELECT * FROM events ${where} ${order} LIMIT ?`).all(...a);
  return rows.map((r) => { let tags = []; try { tags = JSON.parse(r.tags || '[]'); } catch {} return { ...r, tags }; });
}
function eventRegions(city = '') {
  const db = getDb();
  return db.prepare(`SELECT region, COUNT(*) n FROM events WHERE region != '' ${city ? 'AND city = ?' : ''} GROUP BY region ORDER BY n DESC`).all(...(city ? [city] : []));
}

// 活动出席（搭子线下）：返回 { [event_id]: [{user_id,handle,name,avatar}] }，每个活动最多取若干头像
function eventAttendees(eventIds = [], limitEach = 12) {
  const db = getDb();
  if (!eventIds.length) return {};
  const ph = eventIds.map(() => '?').join(',');
  const rows = db.prepare(`
    SELECT a.event_id, u.id AS user_id, u.handle, u.name, u.avatar
  ...
  const map = {};
  for (const r of rows) {
    const arr = (map[r.event_id] = map[r.event_id] || []);
    if (arr.length < limitEach) arr.push({ user_id: r.user_id, handle: r.handle, name: r.name, avatar: r.avatar });
  }
  return map;
}
// 当前用户报名了哪些活动（返回 id 数组）
function myEventIds(userId) {
  if (!userId) return [];
  const db = getDb();
  return db.prepare('SELECT event_id FROM event_attendees WHERE user_id = ?').all(userId).map((r) => r.event_id);
}
// 社区入驻申请：登记/更新意向（同一用户对同一社区只留一条，可改）
function applyCommunity(userId, communityId, note = '', contact = '') {
  const db = getDb();
  db.prepare(`INSERT INTO community_applications (community_id, user_id, note, contact) VALUES (?,?,?,?)
    ON CONFLICT(community_id, user_id) DO UPDATE SET note = excluded.note, contact = excluded.contact, created_at = datetime('now')`)
    .run(communityId, userId, note, contact);
  const count = db.prepare('SELECT COUNT(*) c FROM community_applications WHERE community_id = ?').get(communityId).c;
  return { ok: true, count };
}
// 当前用户已申请入驻的社区 id 列表
function myCommunityApplications(userId) {
  if (!userId) return [];
  const db = getDb();
  return db.prepare('SELECT community_id FROM community_applications WHERE user_id = ?').all(userId).map((r) => r.community_id);
}

// 切换报名（返回 {attending, count}）
function toggleAttend(userId, eventId, city = '') {
  const db = getDb();
  const has = db.prepare('SELECT 1 FROM event_attendees WHERE event_id = ? AND user_id = ?').get(eventId, userId);
  if (has) db.prepare('DELETE FROM event_attendees WHERE event_id = ? AND user_id = ?').run(eventId, userId);
  else db.prepare('INSERT OR IGNORE INTO event_attendees (event_id, user_id, city) VALUES (?,?,?)').run(eventId, userId, city);
  const count = db.prepare('SELECT COUNT(*) c FROM event_attendees WHERE event_id = ?').get(eventId).c;
  return { attending: !has, count };
}
...
const NAV_CATEGORIES = {
  register: '公司注册', accounting: '记账报税', policy: '政策补贴', office: '办公空间',
  legal: '法务合规', finance: '财务银行', marketing: '设计营销', tools: '效率工具', community: '社群',
};
function navLinksByCategory(city = '') {
  const db = getDb();
  const rows = db.prepare(`SELECT id, category, title, url, description, region, official FROM nav_links ${city ? 'WHERE city = ?' : ''} ORDER BY sort ASC, id ASC`).all(...(city ? [city] : []));
  const groups = {};
  for (const r of rows) { (groups[r.category] = groups[r.category] || []).push(r); }
  return groups;
}
function communitiesList(city = '') {
  const db = getDb();
  return db.prepare(`SELECT id, name, region, type, description, link FROM communities ${city ? 'WHERE city = ?' : ''} ORDER BY id ASC`).all(...(city ? [city] : []));
}
function communityById(id) {
  const db = getDb();
  return db.prepare('SELECT id, name, region, type, description, link, city FROM communities WHERE id = ?').get(id) || null;
}
function communityApplicantCount(id) {
  const db = getDb();
  return db.prepare('SELECT COUNT(*) c FROM community_applications WHERE community_id = ?').get(id).c;
}
...
function needsList({ type = '', city = '', limit = 60 } = {}) {
  const db = getDb();
  const w = ["n.status='open'"], a = [];
  if (city) { w.push('n.city = ?'); a.push(city); }
  if (type) { w.push('n.type = ?'); a.push(type); }
  a.push(limit);
  const rows = db.prepare(`
    SELECT n.id, n.type, n.detail, n.region, n.created_at, n.bp_id,
      u.handle, u.name, u.avatar, b.title AS bp_title,
      (SELECT COUNT(*) FROM resources r WHERE r.type = n.type${city ? ' AND r.city = ?' : ''}) AS match_resources
    FROM needs n JOIN users u ON u.id = n.user_id LEFT JOIN bps b ON b.id = n.bp_id
    WHERE ${w.join(' AND ')} ORDER BY n.id DESC LIMIT ?
  `).all(...(city ? [city, ...a] : a));
  return rows;
}
function resourcesList({ type = '', city = '', limit = 60 } = {}) {
  const db = getDb();
  const w = [], a = [];
  if (city) { w.push('r.city = ?'); a.push(city); }
  if (type) { w.push('r.type = ?'); a.push(type); }
  const where = w.length ? 'WHERE ' + w.join(' AND ') : '';
  a.push(limit);
  const rows = db.prepare(`
...
function resourceCounts(city = '') {
  const db = getDb();
  const o = {};
  for (const r of db.prepare(`SELECT type, COUNT(*) n FROM resources ${city ? 'WHERE city = ?' : ''} GROUP BY type`).all(...(city ? [city] : []))) o[r.type] = r.n;
  return o;
}
