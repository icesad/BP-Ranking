...
function getDb() {
  ...
  db.exec(`
    ...
    -- 方向7 OPC 资源中心：资源导航(分类精选链接) + 社区分布
    CREATE TABLE IF NOT EXISTS nav_links (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      category TEXT NOT NULL,         -- register|accounting|policy|office|legal|finance|marketing|tools|community
      title TEXT NOT NULL,
      url TEXT DEFAULT '',
      description TEXT DEFAULT '',
      region TEXT DEFAULT '',
      official INTEGER DEFAULT 0,     -- 1=官方/权威来源
      sort INTEGER DEFAULT 0,
      created_at TEXT DEFAULT (datetime('now'))
    );
    CREATE INDEX IF NOT EXISTS idx_navlinks_cat ON nav_links (category, sort);
    CREATE TABLE IF NOT EXISTS communities (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      name TEXT NOT NULL,
      region TEXT DEFAULT '',
      type TEXT DEFAULT '',           -- OPC社群 | 孵化器 | 俱乐部 | 线上 ...
      description TEXT DEFAULT '',
      link TEXT DEFAULT '',
      created_at TEXT DEFAULT (datetime('now'))
    );
  ...
  // 城市维度（方向7 城市专属内容）：给城市相关表加 city，现有数据默认上海
  for (const t of ['events', 'news', 'nav_links', 'communities', 'needs', 'resources']) {
    try { db.exec(`ALTER TABLE ${t} ADD COLUMN city TEXT DEFAULT '上海'`); } catch {}
  }

  // 活动表补列（对标 OnePilot：类型/价格/时间段）
  for (const m of [
    "ALTER TABLE events ADD COLUMN category TEXT DEFAULT ''",
    "ALTER TABLE events ADD COLUMN price TEXT DEFAULT ''",
    "ALTER TABLE events ADD COLUMN time_text TEXT DEFAULT ''",
  ]) { try { db.exec(m); } catch {} }
  ...
  db.exec(`
    ...
    -- 活动出席/报名（方向6 搭子线下）：谁(用户)报名了哪个活动 → 活动页直接展示搭子头像，方便线下面基
    ...
    CREATE INDEX IF NOT EXISTS idx_evatt_event ON event_attendees (event_id);
    CREATE INDEX IF NOT EXISTS idx_evatt_user ON event_attendees (user_id);
    -- 社区入驻申请（方向7/9）：用户点中意的 OPC 社区 → 一键报名入驻，留意向+联系方式
    ...
    CREATE INDEX IF NOT EXISTS idx_capp_comm ON community_applications (community_id);
  `);
